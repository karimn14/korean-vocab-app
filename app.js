...
function selectCategory(cat) { setState({ category: cat }); }
function selectInputMode(mode) { setState({ inputMode: mode }); }

function startRound() {
  if (!state.direction) return;
  let pool = state.level === 'all' ? WORDS : WORDS.filter(w => w.lv === state.level);
  if (state.category !== 'all') pool = pool.filter(w => w.cat === state.category);
...
function renderStart() {
  ...
  const categories = [
    ...
    { key: 'benda', label: 'Kata Benda' },
    { key: 'pekerjaan', label: 'Pekerjaan' },
    { key: 'kerja', label: 'Kata Kerja' },
  ];
  const inputModes = [
    { key: 'type', label: 'Ketik' },
    { key: 'choice', label: 'Pilihan Ganda' },
  ];
  let poolSize = s.level === 'all' ? WORDS.length : WORDS.filter(w => w.lv === s.level).length;
  if (s.category !== 'all') {
    poolSize = (s.level === 'all' ? WORDS : WORDS.filter(w => w.lv === s.level))
      .filter(w => w.cat === s.category).length;
  }
  const canStart = !!s.direction && poolSize > 0;
  ...
  <div class="start-screen">
    ...
    <div class="panel">
      <div class="panel-label">Pilih kategori kata</div>
      <div class="pill-row">
        ${categories.map(c => `<button class="pill ${s.category === c.key ? 'selected' : ''}" data-category="${c.key}">${esc(c.label)}</button>`).join('')}
      </div>
    </div>

    <button class="btn-primary" id="start-btn" ${canStart ? '' : 'disabled'}>
      ${!s.direction ? 'Pilih arah jawaban dulu' : (poolSize > 0 ? `Mulai Latihan (${Math.min(ROUND_SIZE, poolSize)} kata)` : 'Tidak ada kata untuk kombinasi ini')}
    </button>
  </div>`;
}
